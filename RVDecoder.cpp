...
EncodingType dbt::RVDecoder::getEncodingType(OIInstType InstType) {
  switch(InstType) {
...
extern void dbt::RVDecoder::fillFields(OIInst& I, EncodingType E, Word W) {
  switch (E) {
...
bool CallZero;
OIInst dbt::RVDecoder::decode(uint32_t CodedInst) {
  Word W;
  W.asI_ = CodedInst;
...
std::array<uint32_t, 2> dbt::RVDecoder::getPossibleTargets(uint32_t PC, OIInst Inst) {
  switch (Inst.Type) {
    case dbt::RVDecoder::Jne: 
    case dbt::RVDecoder::Jeqz: 
    case dbt::RVDecoder::Jlez:
    case dbt::RVDecoder::Jgez:
    case dbt::RVDecoder::Jltz:
    case dbt::RVDecoder::Jnez:
    case dbt::RVDecoder::Jgtz:
    case dbt::RVDecoder::Jeq: 
    case dbt::RVDecoder::Bc1f:
    case dbt::RVDecoder::Bc1t:
      return {(PC + (Inst.Imm << 2) + 4), PC + 4};
    case dbt::RVDecoder::Jump: 
    case dbt::RVDecoder::Call:
...
bool dbt::RVDecoder::isControlFlowInst(OIInst Inst) {
  auto T = dbt::RVDecoder::getPossibleTargets(1, Inst);
  if (T[0] == 5 && T[1] == 5) 
    return false;
  return true;
}

bool dbt::RVDecoder::isIndirectBranch(OIInst Inst) {
  switch (Inst.Type) {
    case dbt::RVDecoder::Callr:
    case dbt::RVDecoder::Ijmp:
    case dbt::RVDecoder::Jumpr:
      return true;
    default: 
      return false;
  }
